class Integrations::Csml::ProcessorService < Integrations::BotProcessorService
  ...
  def client_params(session_id)
    {
      bot_id: "chatquick-bot-#{conversation.inbox.id}",
      channel_id: "chatquick-bot-inbox-#{conversation.inbox.id}",
      user_id: session_id
    }
  end
  ...
  def bot_payload
    {
      id: "chatquick-csml-bot-#{agent_bot.id}",
      name: "chatquick-csml-bot-#{agent_bot.id}",
      default_flow: 'chatquick_bot_flow',
      flows: [
        {
          id: "chatquick-csml-bot-flow-#{agent_bot.id}-inbox-#{conversation.inbox.id}",
          name: 'chatquick_bot_flow',
